...
// determined by EDA (Exploratory Data Analysis)
...
// user selectable
#define CITY_LABEL                "SF"   // user selects city label
#define TIME_DIFF_MIN                1   // minimum time difference  allowed
#define TIME_DIFF_MAX            86400   // max time diff allowed, 1 day=86,400 sec
#define EXCLUDE_CALL_TYPE "PASSING CALL"  // officer initiated, on view call

// on off flags
#define EXCLUDE_CALL_TYPE_ON         1   // 1 = exclude, 0 = keep the EXCLUDE_CALL_TYPE
#define FILTER_ON                    1   // 1 = ON and 0 = OFF (bad time diffs)
#define DEBUG_ON                     1
...
// DESCRIPTION: Prints the offsets, widths, and names arrays in DEBUG_ON format.
// RETURNS: void
...
// section 1 //////////////////////////////////////////////////////////////////
    ...
    if (DEBUG_ON)
    {
        printf("[DEBUG_ON] arguments parsed:\n");
        printf("[DEBUG_ON] ptr_data_file   = %s\n", ptr_data_file);
        printf("[DEBUG_ON] ptr_header_file = %s\n", ptr_header_file);
        printf("[DEBUG_ON] num_threads     = %d\n", num_threads);
        printf("[DEBUG_ON] ptr_subfield    = %s\n", ptr_subfield);
        printf("[DEBUG_ON] ptr_subval1     = %s\n", ptr_subval1);
        printf("[DEBUG_ON] ptr_subval2     = %s\n\n", ptr_subval2);
    }
...
// section 4 //////////////////////////////////////////////////////////////////
    ...
    if (DEBUG_ON) printf("[DEBUG_ON] fd_header = %d : %s opened successfully\n\n",
...
// section 5 //////////////////////////////////////////////////////////////////
    ...
    if (DEBUG_ON)
    {
        printf("[DEBUG_ON] fstat() fields:\n");
        printf("[DEBUG_ON] st_dev     = %-15ld ( device file lives on )\n",
               file_info.st_dev);
        printf("[DEBUG_ON] st_ino     = %-15ld ( unique inode number )\n",
               file_info.st_ino);
        printf("[DEBUG_ON] st_nlink   = %-15ld ( number of hard links )\n",
               (long)file_info.st_nlink);
        printf("[DEBUG_ON] st_mode    = %-15o ( permissions in octal )\n",
               file_info.st_mode);
        printf("[DEBUG_ON] st_uid     = %-15d ( user ID of file owner )\n",
               file_info.st_uid);
        printf("[DEBUG_ON] st_gid     = %-15d ( group ID of file owner )\n",
               file_info.st_gid);
        printf("[DEBUG_ON] st_rdev    = %-15ld ( device number )\n",
               file_info.st_rdev);
        printf("[DEBUG_ON] st_size    = %-15ld ( total size in bytes )\n",
               file_info.st_size);
        printf("[DEBUG_ON] st_blksize = %-15ld ( best block size for reading )\n",
               (long)file_info.st_blksize);
        printf("[DEBUG_ON] st_blocks  = %-15ld ( 512-byte blocks on disk )\n",
               file_info.st_blocks);
        printf("[DEBUG_ON] st_atime   = %-15ld ( last time file was read )\n",
               file_info.st_atime);
        printf("[DEBUG_ON] st_mtime   = %-15ld ( last time file was modified )\n",
               file_info.st_mtime);
        printf("[DEBUG_ON] st_ctime   = %-15ld ( permissions changed )\n\n",
               file_info.st_ctime);
...
// section 7 //////////////////////////////////////////////////////////////////
    ...
    if (DEBUG_ON)
    {
        printf("[DEBUG_ON] %d fields parsed from %s\n",
        ...
        printf("[DEBUG_ON] rec_len = %d (sum of all field widths)\n\n", rec_len);
    }
...
// section 8 //////////////////////////////////////////////////////////////////
    ...
    if (DEBUG_ON)
    {
        ...
        printf("[DEBUG_ON] %d fields selected:\n", num_sel_found);
...
// section 9 //////////////////////////////////////////////////////////////////
    ...
    if (DEBUG_ON)
    {
        printf("[DEBUG_ON] offset_received           = %d\n", offset_received);
        printf("[DEBUG_ON] offset_dispatch           = %d\n", offset_dispatch);
        printf("[DEBUG_ON] offset_enroute            = %d\n", offset_enroute);
        printf("[DEBUG_ON] offset_onscene            = %d\n", offset_onscene);
        printf("[DEBUG_ON] offset_call_type_original = %d\n", offset_call_type_original);
        printf("[DEBUG_ON] offset_call_type_final    = %d\n", offset_call_type_final);
        printf("[DEBUG_ON] offset_neighborhood       = %d\n", offset_neighborhood);
        printf("[DEBUG_ON] offset_district           = %d\n", offset_district);
        printf("[DEBUG_ON] size_datetime             = %d\n", size_datetime);
        printf("[DEBUG_ON] size_call_type_original   = %d\n", size_call_type_original);
        printf("[DEBUG_ON] size_call_type_final      = %d\n", size_call_type_final);
        printf("[DEBUG_ON] size_neighborhood         = %d\n", size_neighborhood);
        printf("[DEBUG_ON] size_district             = %d\n\n", size_district);
    }
...
// section 10 /////////////////////////////////////////////////////////////////
    ...
    if (DEBUG_ON) printf("[DEBUG_ON] fd_data = %d : %s opened successfully\n\n",
...
// section 11 /////////////////////////////////////////////////////////////////
    ...
    if (DEBUG_ON) printf("[DEBUG_ON] file size   = %ld bytes\n",  struct_data_info.st_size);
    if (DEBUG_ON) printf("[DEBUG_ON] num_records = %d\n", num_records);
    if (DEBUG_ON) printf("[DEBUG_ON] max_records_per_call_type = %d  "
...
// section 12 /////////////////////////////////////////////////////////////////
    ...
    if (DEBUG_ON) printf("[DEBUG_ON] sizeof(struct struct_call_type_stats) = %zu bytes\n",
    ...
    if (DEBUG_ON)
    {
        printf("[DEBUG_ON] max_call_types = %d "
               "(closest power of 2 >= %d)\n",
               max_call_types, UNIQUE_CALL_TYPES);
        printf("[DEBUG_ON] struct struct_call_type_stats breakdown:\n");
        printf("[DEBUG_ON] ptr_arr_diff_received_to_dispatch : %zu bytes\n",
               sizeof(ptr_arr_stats[0].ptr_arr_diff_received_to_dispatch));
        printf("[DEBUG_ON] ptr_arr_diff_enroute_to_onscene   : %zu bytes\n",
               sizeof(ptr_arr_stats[0].ptr_arr_diff_enroute_to_onscene));
        printf("[DEBUG_ON] ptr_arr_diff_received_to_onscene  : %zu bytes\n",
               sizeof(ptr_arr_stats[0].ptr_arr_diff_received_to_onscene));
        printf("[DEBUG_ON] ptr_label                         : %zu bytes\n",
               sizeof(ptr_arr_stats[0].ptr_label));
        printf("[DEBUG_ON] ptr_call_type                     : %zu bytes\n",
               sizeof(ptr_arr_stats[0].ptr_call_type));
        printf("[DEBUG_ON] count                             : %zu bytes\n",
               sizeof(ptr_arr_stats[0].count));
        printf("[DEBUG_ON] total struct size                 : %zu bytes\n",
               sizeof(struct struct_call_type_stats));
        printf("[DEBUG_ON] total malloc                      : %zu bytes\n",
    ...
    if (DEBUG_ON) printf("[DEBUG_ON] malloc %d call type buckets\n\n",
                      max_call_types);
...
// section 13 /////////////////////////////////////////////////////////////////
    ...
    if (DEBUG_ON) printf("[DEBUG_ON] SUBFIELD = %s  offset = %d  size = %d\n\n",
    ...
    for (rec_num = 0; rec_num < num_records; rec_num++)
    {
        ...
        // exclude the configured call type from all buckets
        if (EXCLUDE_CALL_TYPE_ON &&
            strcmp(ptr_call_type, EXCLUDE_CALL_TYPE) == 0)
            continue;
        ...
        if (FILTER_ON)
        {
            ...
            // skip records with time differences below TIME_DIFF_MIN (bad data)
            if (diff_received_to_dispatch < TIME_DIFF_MIN ||
                diff_enroute_to_onscene   < TIME_DIFF_MIN ||
                diff_received_to_onscene  < TIME_DIFF_MIN)
                continue;
        }
    ...
    if (DEBUG_ON)
    {
        printf("[DEBUG_ON] %d unique call types found\n",   unique_sf);
        printf("[DEBUG_ON] %d total calls processed (%.1f%%)\n",
               total_calls, (double)total_calls / num_records * 100);
        printf("[DEBUG_ON] %d records skipped (missing data) (%.1f%%)\n\n",
...
// section 15 /////////////////////////////////////////////////////////////////
    ...
    if (DEBUG_ON) printf("\n[DEBUG_ON] memory freed, return success\n");
